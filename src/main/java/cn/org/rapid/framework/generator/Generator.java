...
import cn.org.rapid.framework.generator.util.FileHelper;
import cn.org.rapid.framework.generator.util.IOHelper;
...
public class Generator {
	...
	/**
	 * 根据model+template生成对应的文件
	 * @param templateRootDir 模版文件所在路径
	 * @param modelProvider	  data-model
	 * @throws Exception
	 */
	private void generateByModelProvider(final File templateRootDir, final IGeneratorModelProvider modelProvider) throws Exception {
		...
		for(int i = 0; i < templateFiles.size(); i++) {
			...
			//templateFile相对templateRootDir的路径
	 ...
	 * @param outputFilePath	模版文件生成的文件存在路径	
	 * @throws Exception
	 */
	private void generateNewFileOrInsertIntoFile(final IGeneratorModelProvider modelProvider, final Configuration config, final String templateFile,final String outputFilePath) throws Exception {
		...
		System.out.println("[generate]\t template:"+templateFile+" to "+targetFilename +" time="+(e-s) +" miliseconds");
	...
	private Map getTemplateDataModel(final IGeneratorModelProvider modelProvider) throws Exception {
		final Map model = new HashMap();
		//generator.properties的内容,将所有配置项放到model中
		model.putAll(GeneratorProperties.getProperties()); 
